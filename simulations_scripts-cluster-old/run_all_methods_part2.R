...
use_median_at_node_tree_and_forest_weighted_RF <- function ( data, trainRatio = 0.7, 
  ...
  # train
  rg.reg  <- ranger(Y ~ .,num.trees=400,data = trainData,
                    missing.tree.weight=tree.weight, 
                    missing.forest.weight = forest.weight, 
                    seed = seed,impute.missing="median")
...
dataDir<-paste(dataDir,"/",kk,"-evaluation-cor",corVec[kk],"-Rsqr",rsqrVec[kk],"-300/",sep="")

dir.create(paste(dataDir,"/evaluations/MCAR/",sep = ""), showWarnings = FALSE,recursive=TRUE)
dir.create(paste(dataDir,"/evaluations/MAR/",sep = ""), showWarnings = FALSE,recursive=TRUE)
dir.create(paste(dataDir,"/evaluations/MNAR/",sep = ""), showWarnings = FALSE,recursive=TRUE)

saveDir<-paste(dataDir,"/evaluations/",sep="")
